...
@Component({
  ...
  templateUrl: './connect4.round.game.component.html',
  styleUrl: './connect4.round.game.component.css',
  animations: [
    trigger('slide', [
      transition('void => *', [
        style({
...
export class Connect4RoundGameComponent {
  game: Game = new Game(NaN, [], "", "");
  music = new Audio();
  questionModel: ConnectQuestion;
  topQuestionModel: ConnectQuestion;
  answerGroups: Answer[][] = [];
  ...
  constructor(
    ...
    this.questionModel = new ConnectQuestion([], []);
    this.topQuestionModel = new ConnectQuestion([], []);
    this.initScoreboard();
      ...
      for (let player of this.memory.players) {
        if (player.input && player.input !== 'valid') {
          ...
          if (valid) {
            ...
            new Audio('/audio/rounds/connect4/round2correct.mp3').play();
            if (this.timer > 15) {
            ...
            new Audio('/audio/rounds/connect4/round2wrong.mp3').play();
          }
        }
      }
    })
  ...
  private async introduceQuestion() {
    ...
    let answers = this.round.questions[0].data.split('|')[0].split('§').map(every => new Answer(NaN, every.split(';')[0], true, Number(every.split(';')[1])))
    this.round.questions[0].data.split('|')[1].split(';').map(every => new Answer(NaN, every, false, NaN)).forEach(item => answers.push(item));
    answers = this.squares.shuffleArray(answers);
    this.questionModel.connections = this.round.questions[0].data.split('|')[1].split('§').map(every => new Connection(every.split(';')[0], Number(every.split(';')[1])));
    for (let answer of answers) {
  ...
  private async startRotation(rotation: number) {
    ...
    this.music.src = `/audio/rounds/connect4/round2bgm${rotation + 1}.mp3`;
    this.music.play();
    ...
    this.gameService.modifyData(this.memory.gameId, "/connect4", answerString.substring(1)).subscribe(() => {
    })
    this.maxTime = 45;
    this.timer = 45;
  ...
  private async endRound() {
    this.music.src = `/audio/rounds/connect4/round2end.mp3`;
    this.music.play();
  ...
  private async lastTime(groupNumber: number, rotation: number) {
    ...
    this.questionModel.connections = this.questionModel.connections.filter(con => con.explanation !== this.questionModel.connections.filter(con => con.groupNumber === groupNumber)[0].explanation);
    if (this.timer !== 0) {
      let timer = new Audio("/audio/rounds/connect4/round2timer.mp3");
      timer.play();
...
export class ConnectQuestion {
  answers: Answer[];
  connections: Connection[];

  constructor(answers: Answer[], connections: Connection[]) {
...
export class Connection {
  ...
  constructor(explanation: string, groupNumber: number) {
